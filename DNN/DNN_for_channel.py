...
for i in range(bit):
    ...
    model.add(Dense(140, input_dim=2,  kernel_initializer='normal',activation='relu'))                               #加入神經層第一層(輸入14)輸出128 初始化器傳入 激活函數用relu #這邊的input一定要隨著特徵數量更改(pilot數乘2 因為實 虛 分開)
    model.add(Dense(280, input_dim=140,  kernel_initializer='normal',activation='relu'))
    model.add(Dense(140, input_dim=280,  kernel_initializer='normal',activation='relu'))
    model.add(Dense(80, input_dim=140,  kernel_initializer='normal',activation='relu'))
    ...
    # csv.T.to_csv(f'D://MLforSchool//dnn_experiments//channel//mlp_lab4_16qam_10_15_Max_Log_b{i}channel_140_280_140_80.csv')
